...
const createServiceCheckoutSession = async (req, res) => {
  ...
  try {
    ...
    const paymentData = {
      ...
      currency: 'usd',
      paymentMethod: 'card',
      status: 'succeeded', // Auto-complete payment since Stripe handles it
    ...
    // Create Purchase record immediately for direct service purchases
    console.log('Step 6.5: Creating purchase record for service...');
    try {
      ...
      if (!existingPurchase) {
        await Purchase.create({
          ...
          price: amountInCents,
          quantity: 1,
          status: 'completed', // Auto-complete since payment succeeded
          serviceName: service.title
        });
        console.log(`✅ Created purchase record for service: ${service._id}`);
      } else {
        console.log(`⚠️ Purchase record already exists for service: ${service._id}`);
      }
    } catch (purchaseError) {
